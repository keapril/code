                ...
                if cell_content and str(cell_content).strip() != '' and str(cell_content).lower() != 'nan':
                    ...
                    if all_matches:
                        if "台南市立" in hospital_name or "秀傳" in hospital_name:
                            ...
                            if matches_with_spec:
                                ...
                                for code_raw, spec_text in relevant_raw:
                    ...
                    for p_entry in p_info['entries']:
                        for match in found_relevant_matches:
                            final_item = {
                                '醫院名稱': hospital_name,
                                '型號': match['額外型號'] if match['額外型號'] else p_entry['name'],
                                '產品名稱': p_info['產品名稱'],
                                '健保碼': p_info['健保碼'],
                                '院內碼': match['院內碼'],
                                '批價碼': match['批價碼'], 
                                '原始備註': cell_content,
                                '搜尋用字串': p_entry['search_string'] + (f" {match['額外型號'].lower()}" if match['額外型號'] else "")
                            }
